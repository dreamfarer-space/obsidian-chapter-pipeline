...
界面支持键盘聚焦、屏幕阅读器语义、减少动画偏好、强制颜色模式、指针输入和对比度友好的活动指示色。移动端目前明确标记为 `isDesktopOnly: true`，待 Android 与 iOS/iPadOS 实机 smoke test 完成后再开放。
...
CI 还会验证发布元数据/必需资源，并确认生产构建后 `main.js` 不会出现未提交差异。
...
- [`COMMUNITY_SUBMISSION.md`](docs/COMMUNITY_SUBMISSION.md)：社区插件发布与兼容性验证清单
