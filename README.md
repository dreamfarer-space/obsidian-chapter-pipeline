...
The UI supports keyboard focus, screen-reader semantics, reduced-motion preferences, forced-colors mode, pointer input, and contrast-aware active indicators. Mobile distribution is currently disabled with `isDesktopOnly: true` until Android and iOS/iPadOS smoke tests are completed.
...
Repository CI additionally validates release metadata/assets and verifies that a production build does not leave an uncommitted `main.js` diff.
